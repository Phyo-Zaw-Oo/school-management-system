...
public class ContactRestController {
   ...
   // @ResponseBody
    public List<Contact> findByMessageStatus(@RequestParam(name = "status") String status){
        return contactRepository.findContactsByStatus(status);
    }

    @GetMapping("/getAllMessagesByStatus")
   // @ResponseBody
    public List<Contact> findAllMessagesById(@RequestBody Contact contact){
        if(null != contact && null != contact.getStatus()){
            return contactRepository.findContactsByStatus(contact.getStatus());
        }else{
            return List.of();
        }
    }
    ...
    public ResponseEntity<Response> closeMsg(@RequestBody Contact contactReq){
        ...
        if(contact.isPresent()){
            contact.get().setStatus(PhyoSchoolConstants.CLOSE);
